...
fn main() {
    ...
    // CATEGORY 1: DISCOVERY ALGORITHMS (8)
    println!("CATEGORY 1: DISCOVERY ALGORITHMS (8)");
...
fn verify_all_statistics(log: &pm4py::log::EventLog) -> (usize, usize) {
    ...
    use pm4py::statistics::log_stats::log_statistics;
    let log_stats = log_statistics(log);
...
fn verify_all_visualization(log: &pm4py::log::EventLog) -> (usize, usize) {
    ...
    let options = svg_renderer::SvgRenderOptions::default();
    let svg = svg_renderer::render_petri_net_svg(&net, &marking, &options);
    if svg.contains("<svg") {
    ...
    let tree_options = svg_renderer::SvgRenderOptions::default();
    let tree_svg = svg_renderer::render_process_tree_svg(&tree, &tree_options);
...
fn verify_advanced_features(log: &pm4py::log::EventLog) -> (usize, usize) {
    ...
    let _time_pred = RemainingTimePredictor::new(log);
    ...
    let _outcome_pred = OutcomePredictor::new(log, outcome_fn);
