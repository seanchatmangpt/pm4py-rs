...
fn verify_all_model_types() -> (usize, usize) {
    ...
    use pm4py::models::bpmn::BPMNDiagram;
    let bpmn = BPMNDiagram::new("test_process");
    if bpmn.nodes.is_empty() {
        println!("      → BPMN diagram created");
        println!("      ✅ WORKS");
        passed += 1;
    } else {
        println!("      ❌ FAILED");
    ...
    let soundness = PetriNetAnalyzer::check_soundness(&net);
    println!(
        "      → Soundness check: {} places, {} transitions",
        soundness.num_places, soundness.num_transitions
    );
    println!("      ✅ WORKS");
    passed += 1;
    count += 1;
    ...
    use pm4py::models::tree_conversion::petri_net_to_tree;
    let converted_tree = petri_net_to_tree(&net);
    match &converted_tree.root {
    ...
    let task = Task::new("task1", TaskType::Task);
    let gateway = Gateway::new("gateway1", GatewayType::Exclusive);
    println!("      → BPMN Task and Gateway created");
    println!("      ✅ WORKS");
    passed += 1;
    count += 1;
...
fn verify_writer_capabilities() -> (usize, usize) {
    ...
    use pm4py::io::json::JsonEventLogWriter;
