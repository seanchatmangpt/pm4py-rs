...
fn call_conformance_diagnostics_alignments_official() {
    ...
    let result = pyo3::Python::with_gil(|py| {
        pm4py::python::generated::conformance::call_conformance_diagnostics_alignments(
            py, &log, &net,
        )
    });
...
fn call_conformance_diagnostics_footprints_official() {
    ...
    let net = ab_petri_net();
    let result = pyo3::Python::with_gil(|py| {
        pm4py::python::generated::conformance::call_conformance_diagnostics_footprints(
            py, &log, &net,
        )
    });
...
fn call_conformance_diagnostics_token_based_replay_official() {
    if !pm4py_available() {
        eprintln!(
            "SKIP call_conformance_diagnostics_token_based_replay_official: pm4py not available"
        );
        return;
    }
    let log = two_activity_log();
...
fn call_discover_petri_net_alpha_official() {
    ...
    assert!(
        !discovered.places.is_empty(),
        "discover_petri_net_alpha: PetriNet must have places"
    );
    assert!(
        !discovered.transitions.is_empty(),
...
fn call_read_xes_official() {
    ...
    let result =
        pyo3::Python::with_gil(|py| pm4py::python::generated::io::call_read_xes(py, XES_PATH));
...
fn call_write_xes_official() {
    ...
    let result =
        pyo3::Python::with_gil(|py| pm4py::python::generated::io::call_write_xes(py, &log, path));
