...
//! use std::path::Path;
//!
//! let reader = StreamingJsonReader::new();
...
pub struct StreamingJsonReader {
    /// Case ID column name
...
impl StreamingJsonReader {
    ...
    /// Read streaming JSON (JSONL format - one JSON object per line)
            ...
            match serde_json::from_str::<Value>(&line) {
                Ok(json) => {
                    ...
                    let case_id = json[&self.case_column]
                        .as_str()
                        .ok_or_else(|| anyhow!("Line {}: Missing case column '{}'. Tip: check column name in config or data file. See docs/TROUBLESHOOTING.md#missing-column", line_num, self.case_column))?
                        .to_string();

                    let activity = json[&self.activity_column]
                        .as_str()
                        .ok_or_else(|| anyhow!("Line {}: Missing activity column '{}'. Tip: check column name in config or data file. See docs/TROUBLESHOOTING.md#missing-column", line_num, self.activity_column))?
                        .to_string();

                    let timestamp_str = json[&self.timestamp_column]
                        .as_str()
                        .ok_or_else(|| anyhow!("Line {}: Missing timestamp column '{}'. Tip: check column name in config or data file. See docs/TROUBLESHOOTING.md#missing-column", line_num, self.timestamp_column))?;
                    ...
                    let timestamp = DateTime::parse_from_rfc3339(timestamp_str)
                        ...
                        .map_err(|_| anyhow!("Line {}: Invalid timestamp format. Expected RFC3339 or RFC2822. Example: 2026-03-25T10:30:00Z. See docs/TROUBLESHOOTING.md#timestamp-parsing", line_num))?;
...
impl Default for StreamingJsonReader {
    fn default() -> Self {
        Self::new()
    }
}
...
impl StreamingJsonWriter {
    ...
    /// Write EventLog as streaming JSON (JSONL format)
...
impl LogReader for StreamingJsonReader {
...
mod tests {
    ...
    #[test]
    fn test_streaming_json_round_trip() {
        let mut log = EventLog::new();
    ...
    #[test]
    fn test_streaming_json_validation_valid_event() {
        let writer = StreamingJsonWriter::new();
    ...
    #[test]
    fn test_streaming_json_validation_missing_field() {
        let writer = StreamingJsonWriter::new();
        let event_json = serde_json::json!({
            "case_id": "case_1",
            "activity": "start"
    ...
    #[test]
    fn test_streaming_json_validation_non_object() {
        let writer = StreamingJsonWriter::new();
