...
pub fn call_check_is_fitting(py: Python<'_>, log: &EventLog) -> PyResult<()> {
...
pub fn call_conformance_declare(py: Python<'_>, log: &EventLog) -> PyResult<f64> {
    let pm4py = py.import("pm4py")?;
    let py_log = eventlog_to_pm4py(py, log)?;
...
pub fn call_conformance_etot(py: Python<'_>, ocel: &PyAny) -> PyResult<f64> {
    let pm4py = py.import("pm4py")?;
...
///
/// Call style: `log_and_model` → convert: `list_dict`
pub fn call_conformance_log_skeleton(py: Python<'_>, log: &EventLog) -> PyResult<f64> {
    let pm4py = py.import("pm4py")?;
    let py_log = eventlog_to_pm4py(py, log)?;
...
///
/// Call style: `ocel_only` → convert: `list_dict`
pub fn call_conformance_ocdfg(py: Python<'_>, ocel: &PyAny) -> PyResult<f64> {
    let pm4py = py.import("pm4py")?;
...
///
/// Call style: `ocel_only` → convert: `list_dict`
pub fn call_conformance_otg(py: Python<'_>, ocel: &PyAny) -> PyResult<f64> {
    let pm4py = py.import("pm4py")?;
...
pub fn call_conformance_temporal_profile(py: Python<'_>, log: &EventLog) -> PyResult<()> {
    let pm4py = py.import("pm4py")?;
    let py_log = eventlog_to_pm4py(py, log)?;
...
pub fn call_fitness_alignments(py: Python<'_>, log: &EventLog, net: &PetriNet) -> PyResult<f64> {
    let pm4py = py.import("pm4py")?;
    // WvdA soundness: bound runtime — cap at 1,000 traces
    const MAX_TRACES: usize = 1_000;
    let truncated: EventLog;
...
pub fn call_fitness_footprints(py: Python<'_>, log: &EventLog, net: &PetriNet) -> PyResult<f64> {
    let pm4py = py.import("pm4py")?;
    // WvdA soundness: bound runtime — cap at 1,000 traces
    const MAX_TRACES: usize = 1_000;
    let truncated: EventLog;
...
pub fn call_fitness_token_based_replay(
    ...
    let (py_net, py_im, py_fm) = rust_petri_to_pm4py(py, net)?;
    let result =
        pm4py.call_method1("fitness_token_based_replay", (py_log, py_net, py_im, py_fm))?;
    // pm4py returns dict {"average_trace_fitness": f64, ...}; extract the key
    result.get_item("average_trace_fitness")?.extract::<f64>()
}
...
///
/// Call style: `log_and_net` → convert: `f64`
pub fn call_generalization_tbr(py: Python<'_>, log: &EventLog, net: &PetriNet) -> PyResult<f64> {
    let pm4py = py.import("pm4py")?;
    // WvdA soundness: bound runtime — cap at 1,000 traces
    const MAX_TRACES: usize = 1_000;
    let truncated: EventLog;
...
///
/// Call style: `log_and_net` → convert: `f64`
pub fn call_precision_alignments(py: Python<'_>, log: &EventLog, net: &PetriNet) -> PyResult<f64> {
    let pm4py = py.import("pm4py")?;
    // WvdA soundness: bound runtime — cap at 1,000 traces
    const MAX_TRACES: usize = 1_000;
    let truncated: EventLog;
...
///
/// Call style: `log_and_net` → convert: `f64`
pub fn call_precision_footprints(py: Python<'_>, log: &EventLog, net: &PetriNet) -> PyResult<f64> {
    let pm4py = py.import("pm4py")?;
    // WvdA soundness: bound runtime — cap at 1,000 traces
    const MAX_TRACES: usize = 1_000;
    let truncated: EventLog;
...
pub fn call_replay_prefix_tbr(py: Python<'_>, log: &EventLog) -> PyResult<()> {
    let pm4py = py.import("pm4py")?;
    let py_log = eventlog_to_pm4py(py, log)?;
